...
/**
 * task d'intégration avec recyclage de paquet rejeté lorsque un paquet de taille supérieure à 1 est rejeté, cette implémentation
 * re-soumet ligne par ligne les éléments du paquet rejeté pour ne rejeter que les éléments qui posent problème
 */
public class TaskIntegrationUnitReject extends TaskIntegrationImpl
{
  /**
   ...
   * logger
   */
  private static final Logger LOGGER = Logger.getLogger(TaskIntegrationUnitReject.class);
   ...
   */
  public TaskIntegrationUnitReject(AbstractBatch pParent, INTEGRATION pBean) throws TaskException
  {
    super(pParent, pBean);
  }
  ...
  private void buildRetryProvider(RejectResponse pResponse) throws ProviderException
  {
    ...
    _retryProviderReader = new ProviderReaderImpl(myXmlStringProvider, 1, BatchUtil.getKeepRootElementPrefix(_inputProviderReader), BatchUtil.getIgnoreRootNamespace(_inputProviderReader));
  }
